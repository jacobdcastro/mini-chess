...
const Board = () => {
    ...
    const updateStateAfterMove = newGameData => {
        setGame(Object.assign(Object.assign({}, game), newGameData)); // set game state
    ...
    useInput((input, key) => {
        ...
        if (selectedPossibleMoves.length === 0) {
            setPlayer(Object.assign(Object.assign({}, player), { cursorPosition: moveCursor(input, key, player) }));
        }
        else {
        ...
        if (selectedPossibleMoves.length > 0 && key.return) {
            ...
            if (player.color === 'w') {
                selectedPieceData = pieces.active.white.find(p => p._id === selectedPiece._id);
                selectedPieceNewCoords = { x: curPos.x, y: curPos.y };
            }
            else {
                selectedPieceData = pieces.active.black.find(p => p._id === selectedPiece._id);
                selectedPieceNewCoords = { x: curPos.x, y: curPos.y };
            }
            ...
            highlightPossibleMoves(pieces, player, setPlayer, setSelectedPossibleMoves, board);
        }
    });
    const displayBoard = () => {
        let rows = [];
        ...
        if (player.color === 'w') {
            for (let i = 7; i >= 0; i--) {
                rows.push(<Row selectedPossibleMoves={selectedPossibleMoves} key={i} rowIndex={i} spaces={board[i]}/>);
            }
        }
        else {
            for (let i = 0; i < 8; i++) {
    ...
    const showCoordinates = type => {
        if (type) {
            const yAxisElements = [];
            for (let i = 1; i <= 8; i++) {
                yAxisElements.push(<Color key={i} cyan>
            {i}
          </Color>);
            }
            ...
            const xAxisElements = [];
            for (let i = 0; i < 8; i++) {
                xAxisElements.push(<Color key={i} cyan>
            {' '}
    ...
    return (<Box marginRight={3} flexDirection="column">
