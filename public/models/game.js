...
class Game {
    ...
    createInitBoard() {
        ...
        for (let y = 0; y < 8; y++) {
            for (let x = 0; x < 8; x++) {
                const piece = allPieces.find(({ position }) => position.x === x && position.y === y);
                newBoard[y][x] = new Space(y, x, piece ? { type: 'ref', _id: piece._id } : null);
            }
        }
        return newBoard;
    }
    updateGame(move) {
        ...
        for (let y = 0; y < 8; y++) {
            for (let x = 0; x < 8; x++) {
                const piece = allPieces.find(({ position }) => position.x === x && position.y === y);
                updatedBoard[y][x] = new Space(y, x, piece && { type: 'ref', _id: piece._id });
            }
        }
