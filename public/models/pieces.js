...
class Pieces {
    ...
    initializeMove(move) {
        const { movedPieceIsWhite, movedPieceId, startingLocation, endingLocation, capturedPieceId, } = move;
        let selectedPiece;
        let capturedPiece;
        ...
        if (capturedPieceId !== null) {
            if (movedPieceIsWhite) {
                ...
                this.active.black = this.active.black.filter(p => p._id !== capturedPieceId);
            }
            else {
                capturedPiece = this.active.white.find(p => p._id === capturedPieceId);
                this.captured.white.push(capturedPiece);
                this.active.white = this.active.white.filter(p => p._id !== capturedPieceId);
            }
        }
    }
    addInitialPawns() {
