...
const checkCastling = ({ active }) => {
    ...
    if (!wRook1.hasMoved && !wKing.hasMoved)
        str += 'K'; // kingside
    if (!wRook2.hasMoved && !wKing.hasMoved)
        str += 'Q'; // queenside
    ...
    if (!bRook2.hasMoved && !bKing.hasMoved)
        str += 'k'; // kingside
    if (!bRook1.hasMoved && !bKing.hasMoved)
        str += 'q'; // queenside
    return str;
};
